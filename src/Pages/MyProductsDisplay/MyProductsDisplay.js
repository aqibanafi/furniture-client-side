...
const MyProductsDisplay = ({ products }) => {
    ...
    return (
        <div className='shadow-xl flex flex-col justify-between p-10 rounded-lg bg-accent w-[400px] mb-5'>
            <img className='w-80 h-80 mb-5 rounded-xl' src={picture} alt="" />
            <div className='flex justify-between items-center'>
                <p className='text-2xl font-semibold text-primary mb-5'>{name}</p>
                <button onClick={() => handleMakeSold(_id)} disabled={products.status === "Sold"} className={`px-5 py-2 text-white ${`product.status ? "Sold" bg-green-500 : bg-red-500`} `}>{products.status}</button>
            </div>
            ...
            <div className='flex gap-5 mt-10'>
                <Link to={`/dashbaord/editproduct/${_id}`}><button className="btn btn-active">Edit</button></Link>
                <button onClick={() => handleAdvertise(_id)} className={`btn btn-success ${`bg-green-700 text-white`}`} disabled={products.advertiseStatus === "Advertised"}>Advertise</button>
